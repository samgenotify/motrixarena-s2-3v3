...
def do_forward(rs, gx):
    ...
    # v25: sideline recovery — when ball near sideline in opponent half
    near_sideline = abs(by) > 2.0 and bx > 1.0

    if near_sideline and bd > 0.5:
        # Phase 1: position BEHIND ball on the sideline side
        # This means going to (bx-0.8, by + sign(by)*0.3)
        # From there, a push toward goal will naturally angle toward center
        behind_x = bx - 0.8
        ...
        # MEDIUM: approach from behind ball with stronger offset + centering
        # v30: larger offset (1.0m) + y_bias to pull approach toward center side
        apx = bx - gdx * 1.0
        apy = by - gdy * 1.0 + y_bias * 0.5
...
class MC:
    ...
    def run(self):
        period = 1.0 / self.hz
        log.info(f"[MC v30] Strategy @ {self.hz}Hz, ZMQ @ {self.comm.zmq_hz}Hz, "
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="MotrixArena S2 3v3 Multi-Controller v25")
