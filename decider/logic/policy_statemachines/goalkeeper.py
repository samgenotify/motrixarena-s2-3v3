...
class GoalkeeperStateMachine:
    ...
    def read_params(self):
        ...
        self.activate_save = goalkeeper_config.get("activate_save", False)
    ...
    def do_clearance(self):
        ...
        if abs(self.agent.get_self_yaw() > 90):
            self.agent._state_machine_runners['dribble'](aim_yaw=yaw)
        else:
            # self.agent._state_machine_runners['chase'](distance=0)
            self.agent._state_machine_runners['dribble'](aim_yaw=yaw)
    ...
    def goal_risk_high(self):
        ...
        if not self.activate_save:
            self.logger.info("[GOALKEEPER FSM] Saving is not activated, return.")
            return False
